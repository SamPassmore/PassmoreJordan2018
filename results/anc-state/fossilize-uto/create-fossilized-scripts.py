...
# get phylogeny
def get_taxa(kc):
  tree = NexusReader(dir+'/data2/'+kc+'.bttrees')
  taxa = []
  for t in tree.taxa:
      taxa.append(t)
  taxa_str = ' '.join(taxa)
...
#PBS -o /panfs/panasas01/arch/sp16194/terminology-anc-state/fossilize-uto/logs/utoaztecan-{codes[str(f)]}-fossilize-output.txt
#PBS -e /panfs/panasas01/arch/sp16194/terminology-anc-state/fossilize-uto/logs/utoaztecan-{codes[str(f)]}-fossilize-error.txt
...
Stones 100 1000
'''
    taxa_str = get_taxa("utoaztecan")
    part_2 = f'''AddTag root {taxa_str}
Fossil Node01 root {f}
'''
    part_3 = f'''LogFile fossilize-uto/output/{codes[str(f)]}_$DATE;
run;
ANSWERS'''
    job_file = part_1+part_2+part_3
    with open(f"/Users/sp16194/Desktop/Projects_Git/terminology-anc-state/fossilize-uto/jobs/job-utoaztecan-{codes[str(f)]}.sh", "w") as text_file:
