...
kable(quotes[,1:6], 
      caption = "Table S3: hypotheses with quotes, references and page numbers", format = format_change) %>% 
 kable_styling(full_width = TRUE)
```
...
for(k in bantu_kincodes){
  ...
  print(
    lh_table %>% 
    kable(format = format_change, caption = "Table S9: Marginal log-likelihood for three runs of each fossilized terminology in Bantu", digits = 3) %>%
    kable_styling(bootstrap_options = c("striped", "bordered"))
  )
