...
We fossilize the ancestral state for each possible terminology within Bantu in an attempt to determine the most plausible ancestral state. Below are the marginal log-likelihoods for these models and the result with comparisons to the most likely ancestral state, Iroquoian. 
...
print(
  bf %>% 
    kable(format = format_change, 
          caption = "Table S10: Pariwise Bayes factor between Iroquois and all other possible states.",
          digits = 3, row.names = FALSE) %>%
    kable_styling(bootstrap_options = c("striped", "bordered"))
)
```

### Fossilized Uto-Aztecan
#### MCMC Review

As with Bantu, we fossilize the ancestral state for each possible terminology within UtoAztecan to determine the most plausible ancestral state. Below are the marginal log-likelihoods for these models, followed by a BayesFactor comparisons between the most likely ancestral state & all other possible states. 

```{r fossilized-uto}
files = list.files('results/anc-state/fossilize-uto/output', pattern = "Log.txt", 
                   full.names = TRUE)

stones_files = list.files('results/anc-state/fossilize-uto/output', pattern = "Stones.txt", 
                   full.names = TRUE)

uto_kincodes = stones_files %>% 
  basename() %>% 
  str_split("_") %>%
  sapply(extract2, 1) %>% 
  unique()

lh_table = vector(length = length(uto_kincodes))
for(i in 1:length(uto_kincodes)){
  k = uto_kincodes[i]
  idx = str_detect(files, as.character(k))
  logs = map(files[idx], bt_read.log)
...
lh_table2 = data.frame(terminology = uto_kincodes, MLL = lh_table, row.names = NULL)


print(
    lh_table2 %>% 
    kable(format = format_change, caption = "Table S11: Marginal log-likelihood for each fossilized terminology in Uto Aztecan", digits = 3, col.names = c("terminology", "MLL")) %>%
    kable_styling(bootstrap_options = c("striped", "bordered"))
  )
```
...
Bayes factor calculations show that even when fossilizing to each possible root - there is no preference for a particular ancestral state. 


```{r fossilized-uto-bf, results='asis'}
lh = lh_table2[,2]
bf = outer(lh, lh, function(x,y) 2 * (x - y))
dimnames(bf) = list(lh_table2[,1], lh_table2[,1])
bf = data.frame(terminology = colnames(bf) , bf = bf[,"Hawaiian"])
colnames(bf) = c("terminology", "Bayes' Factor")

print(
  bf %>% 
    kable(format = format_change, 
          caption = "Table S12: Pariwise Bayes factor between Iroquois and all other possible states.",
          digits = 3, row.names = FALSE) %>%
    kable_styling(bootstrap_options = c("striped", "bordered"))
)
```
...
print(
  recnode %>% 
    kable(format = format_change, 
          caption = "Table S13: Ancestral state for PMP",
...
print(
  anc_states %>% 
    kable(format = format_change, 
          caption = 
            "Table S17: Ancestral state of Bantu with missing data included",
...
kable(m[complete.cases(m),], col.names = 
      ...
      caption = "Table S18: MCMC settings for both dependent and independent analyses", format = format_change) %>% 
  kable_styling()
```
...
for(i in seq_along(m$job)){
  ...
  idx = str_detect(files_1, paste0(hypotheses, "-[i|d]"))
  f1 = files_1[idx]
  
  idx = str_detect(files_2, paste0(hypotheses, "-[i|d]"))
  f2 = files_2[idx]
  
  idx = str_detect(files_3, paste0(hypotheses, "-[i|d]"))
  f3 = files_3[idx] 
  ...
  idx = str_detect(stones_1, paste0(hypotheses, "-[i|d]"))
  s1_f = stones_1[idx]
  
  idx = str_detect(stones_2, paste0(hypotheses, "-[i|d]"))
  s2_f = stones_2[idx]
  
  idx = str_detect(stones_3, paste0(hypotheses, "-[i|d]"))
  s3_f = stones_3[idx] 
